...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-admin-changelist-charts runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-admin-changelist-charts.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_admin_changelist_charts")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-admin-changelist-charts tests")
...
def release(c, bumpsize=''):
    ...
    import dj_admin_changelist_charts
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_admin_changelist_charts.__version__))
    c.run("git push --tags")
